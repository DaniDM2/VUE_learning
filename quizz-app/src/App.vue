<template>
  ...
  <!-- <transition name="fade" mode="out-in">
    <div v-if="quizStore.quizFinished" class="p-2 flex flex-col items-center">
      ...
      <div class="mt-4 px-2">
        <ul>
          <li class="mt-8" v-for="row in quizStore.result" :key="row.id">
            ...
            <div class="text-2xl font-ssp capitalize mt-4">
              <div class="feedback flex items-center">
                ...
                <span
                  class="px-3.5 py-3 m-2 bg-black text-white rounded-full flex justify-center items-center max-w-[3rem]"
                  ><i
                    :class="['pi', row.answerisRight ? 'pi-check' : 'pi-times']"
                  ></i
                ></span>
              </div>
      ...
      <div class="mt-4">
        <button
          class="text-4xl font-ssp font-bold bg-black text-white capitalize px-4 py-2"
